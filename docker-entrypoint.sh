...
# Bind to all interfaces by default - correct for Docker.
# Override SERVER_IP in .env only if you need to bind to a specific interface.
...
# generation entirely.
# =============================================================================
# Config is always written to /server/config/server.cfg so the user can
# inspect and edit it. It is then symlinked into /server/codam/ where
# the engine expects it.
CONFIG_CFG="/server/config/server.cfg"
GENERATED_CFG="/server/codam/server.cfg"

if [ -f "$CONFIG_CFG" ] && [ ! -L "$CONFIG_CFG" ]; then
    # A real hand-crafted file exists (not a symlink we created) - use it.
    log "Found hand-crafted ${CONFIG_CFG} - using it, skipping generation."
    ln -sf "$CONFIG_CFG" "$GENERATED_CFG"
else
    log "Generating ${CONFIG_CFG} from env vars ..."
...
EOF
    # Symlink into /server/codam/ where the engine looks for it
    ln -sf "$CONFIG_CFG" "$GENERATED_CFG"
    log "Config written to ${CONFIG_CFG} and linked to ${GENERATED_CFG}"
fi
